...
class TantalusAssociationAdminForm(forms.ModelForm):
    """Tantalus Association Admin Form."""

    tantalus_id = forms.ChoiceField(required=True)

    def __init__(self, *args, **kwargs):
        """Initialize Tantalus Association Admin Form by getting relation data from Tantalus."""
        super(TantalusAssociationAdminForm, self).__init__(*args, **kwargs)
        if not TantalusClient.can_create_client():
            return
        try:
            tantalus_client = get_tantalus_client()
            choices = [(x["id"], x["name"]) for x in tantalus_client.get_relations()]
            self.fields["tantalus_id"].choices = choices
        except TantalusException as e:
            logging.error(
                "The following Exception occurred while trying to access Tantalus on the administration "
