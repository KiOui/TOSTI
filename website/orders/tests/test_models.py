...
class OrderModelTests(TestCase):
    ...
    def test_product_user_order_amount(self):
        product = models.Product.objects.create(
            name="Test no restriction", max_allowed_per_shift=2, current_price=0.8
        )
        self.assertTrue(product.user_can_order_amount(self.normal_user, self.shift))
        self.assertTrue(
            product.user_can_order_amount(self.normal_user, self.shift, amount=2)
        )
        self.assertFalse(
            product.user_can_order_amount(self.normal_user, self.shift, amount=3)
        )
        models.Order.objects.create(
            user=self.normal_user, shift=self.shift, product=product
        )
        self.assertTrue(product.user_can_order_amount(self.normal_user, self.shift))
        self.assertFalse(
            product.user_can_order_amount(self.normal_user, self.shift, amount=2)
        ...
        models.Order.objects.create(
            user=self.normal_user, shift=self.shift, product=self.product_2
        )
        self.assertTrue(product.user_can_order_amount(self.normal_user, self.shift))
    ...
    def test_product_user_max_order_amount_no_restriction(self):
        ...
        self.assertEqual(
            product.user_max_order_amount(self.normal_user, self.shift), None
        )
        models.Order.objects.create(
            user=self.normal_user, shift=self.shift, product=product
        )
    ...
    def test_active_venue_validator(self):
        ...
        self.assertRaises(
            ValidationError, models.active_venue_validator, self.order_venue
        )
    ...
    def test_shift_save_create_finalized(self, _make_finalized_mock, _clean_mock):
        ...
        models.Shift.objects.create(
            venue=self.order_venue, start=start, end=end, finalized=True
        )
        _make_finalized_mock.assert_called()
    ...
    def test_shift_number_of_restricted_orders(self):
        ...
        models.Order.objects.create(
            user=self.normal_user, product=product_unrestricted, shift=self.shift
        )
        self.assertEqual(self.shift.number_of_restricted_orders, 1)
    ...
    def test_shift_max_orders_total_string(self):
        ...
        shift = models.Shift.objects.create(
            venue=self.order_venue, start=start, end=end, max_orders_total=13
        )
        self.assertEqual(
            shift.max_orders_total_string, "{}".format(shift.max_orders_total)
        )
    ...
    def test_shift_capacity(self):
        self.assertEqual(
            self.shift.capacity, "{} / {}".format(0, self.shift.max_orders_total_string)
        )
        models.Order.objects.create(
            user=self.normal_user, product=self.product, shift=self.shift
        )
        self.assertEqual(
            self.shift.capacity, "{} / {}".format(1, self.shift.max_orders_total_string)
        )
        product_unrestricted = models.Product.objects.create(
            name="Unrestricted product", current_price=5, ignore_shift_restrictions=True
        )
        ...
        self.assertEqual(
            self.shift.capacity, "{} / {}".format(2, self.shift.max_orders_total_string)
        )
    ...
    def test_shift_date(self):
        ...
        shift.end = timezone.make_aware(
            datetime.datetime(year=2022, month=3, day=5, hour=13, minute=30)
        )
        shift.save()
    ...
    def test_shift_human_readable_start_end_time(self):
        ...
        with self.subTest("Start end on same day"):
            ...
            self.assertEqual(
                ...
                "{}, 12:15 until 13:30".format(
                    start.strftime(models.Shift.HUMAN_DATE_FORMAT)
                ),
            )
        ...
        with self.subTest("Start end on today date"):
            ...
            shift_2 = models.Shift.objects.create(
                venue=order_venue_2, start=start_today, end=end_today
            )
    ...
    def test_shift__make_finalized(self):
        start = timezone.make_aware(
            datetime.datetime(year=2022, month=4, day=2, hour=12, minute=15)
        )
        end = timezone.make_aware(
            datetime.datetime(year=2022, month=4, day=2, hour=13, minute=30)
        )
        shift = models.Shift.objects.create(
            venue=self.order_venue, start=start, end=end
        )
    ...
    def test_shift_shift_done(self):
        ...
        models.Order.objects.create(
            ...
            shift=self.shift,
            type=models.Order.TYPE_SCANNED,
        )
        self.assertTrue(self.shift.shift_done)

    def test_shift__clean_no_venue_id(self):
        start = timezone.make_aware(
            datetime.datetime(year=2022, month=3, day=2, hour=12, minute=15)
        )
        end = timezone.make_aware(
            datetime.datetime(year=2022, month=3, day=2, hour=13, minute=30)
        )
        shift = models.Shift.objects.create(
            venue=self.order_venue, start=start, end=end
        )
    ...
    def test_shift__clean_finalized_not_done_shift(self):
        ...
        models.Order.objects.create(
            user=self.normal_user, product=self.product, shift=shift
        )
        shift.finalized = True
        self.assertRaises(ValidationError, shift._clean)
    ...
    def test_shift__clean_overlapping_shifts(self):
        ...
        with self.subTest("Shift inside other shift"):
            ...
            shift_inside_other_shift = models.Shift.objects.create(
                venue=order_venue_2,
                start=start_shift_inside_other_shift,
                end=end_shift_inside_other_shift,
            )
        ...
        with self.subTest("Shift before other shift overlapping"):
            ...
            shift_before_other_shift = models.Shift.objects.create(
                venue=order_venue_2,
                start=start_shift_before_other_shift,
                end=end_shift_before_other_shift,
            )
        ...
        with self.subTest("Shift after other shift overlapping"):
            ...
            shift_after_other_shift = models.Shift.objects.create(
                venue=order_venue_2,
                start=start_shift_after_other_shift,
                end=end_shift_after_other_shift,
            )
    ...
    def test_shift_user_can_order_amount(self):
        ...
        shift = models.Shift.objects.create(
            venue=self.order_venue, start=start, end=end, max_orders_per_user=None
        )
        self.assertTrue(shift.user_can_order_amount(self.normal_user, 15))
        shift.max_orders_per_user = 2
        shift.save()
        models.Order.objects.create(
            user=self.normal_user, shift=shift, product=self.product
        )
    ...
    def test_available_product_filter(self):
        ...
        self.assertRaises(
            ValidationError,
            models.available_product_filter,
            test_product_unavailable.pk,
        )
        self.assertTrue(models.available_product_filter(test_product_available))
        self.assertRaises(
            ValidationError, models.available_product_filter, test_product_unavailable
        )

    def test_order___str__(self):
        order = models.Order.objects.create(
            user=self.normal_user, shift=self.shift, product=self.product
        )
        self.assertEqual(
            order.__str__(),
            "{} for {} ({})".format(self.product, self.normal_user, self.shift),
        )
